...
void handleFindResult(ClientTransferState *state, PacketPayload *payload) {
    ...
    log_success("keyLength:%d\nkey:%s\nValueLength:%d\nvalue:%s\n", transferCommandPayload->keyLength,
                transferCommandPayload->key,
...
void handleSaveResult(ClientTransferState *state,PacketPayload *payload){
    SaveResult *result=(SaveResult*)payload->src;
    if(result->flag){
        log_success("Server save data success\n");
    }else{
        log_error("Server save data failed\n");
    }
}
...
void initRequestProcessor(void) {
    ...
    setDataHandler(SAVE_RESULT,handleSaveResult);
}
